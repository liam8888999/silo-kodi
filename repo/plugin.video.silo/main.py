...
def _watch_party_monitor(
    ...
    def handle_local_transport_event(action, player):
        """Handle an immediate Kodi play/pause callback for a controllable guest."""
        nonlocal last_observed_paused
        ...
        # Ignore callbacks generated by our own server command while its guard
        # is active. This prevents remote commands from echoing back as new
        # guest requests.
    ...
    class _WatchPartyPlayer(xbmc.Player):
        ...
        def onPlayBackPaused(self):
            if (
                ...
                # Guest transport is controlled locally in this mode. Send the
                # user's pause immediately; polling remains as a fallback for
                # Kodi skins/remotes that do not reliably deliver this callback.
                handle_local_transport_event("pause", self)
                return
        ...
        def onPlayBackResumed(self):
            if (
                ...
                # Same path as pause: callback is immediate, polling protects
                # against a missed Kodi callback.
                handle_local_transport_event("play", self)
                return
        ...
        while (
            ...
            if (
                ...
                if time.time() < transport_guard_until:
                    ...
                    if handle_local_transport_event(action, player):
                        last_observed_paused = actual_paused
            else:
                last_observed_paused = None
